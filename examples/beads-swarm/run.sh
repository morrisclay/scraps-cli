#!/bin/bash
# Beads Swarm - Execute any PRD with N parallel agents
#
# Uses Beads for task management and Scraps for file coordination.
#
# Usage:
...
# Shared work directory for beads (all agents use the same database)
BEADS_WORK_DIR=""
...
cleanup() {
    ...
    if [ -n "$BEADS_WORK_DIR" ] && [ -d "$BEADS_WORK_DIR" ]; then
        echo -e "${BLUE}Beads database preserved at: $BEADS_WORK_DIR${NC}"
    fi
...
print_banner() {
    ...
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║                                                            ║"
    echo "║   🧵 BEADS SWARM - PRD Executor with Beads + Scraps 🧵    ║"
    echo "║                                                            ║"
    echo "║   Beads = Brain (tasks & dependencies)                    ║"
    echo "║   Scraps = Hands (file coordination & commits)            ║"
    echo "║                                                            ║"
    echo "╚════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
show_help() {
    echo "Beads Swarm - Execute any PRD with parallel AI agents"
    echo ""
    echo "Uses Beads for task management and Scraps for file coordination."
    echo ""
    ...
    echo ""
    echo "Integration:"
    echo "  Beads: Tasks are created as Beads issues with dependencies"
    echo "  Scraps: Files are claimed/released to prevent conflicts"
    echo ""
    echo "After completion:"
    echo "  cd \$BEADS_WORK_DIR && bd list --pretty  # View task graph"
    echo "  scraps clone store/repo                  # Clone completed code"
}
...
check_prereqs() {
    ...
    echo -e "${GREEN}✓ Beads CLI installed ($(bd --version 2>/dev/null | head -1 || echo 'unknown'))${NC}"
...
main() {
    ...
    # Create shared beads work directory
    BEADS_WORK_DIR=$(mktemp -d -t beads-swarm-XXXXXX)
    export BEADS_WORK_DIR
    ...
    echo ""
    echo -e "${CYAN}View Beads task graph:${NC}"
    echo ""
    echo -e "  ${BOLD}${YELLOW}cd $BEADS_WORK_DIR && bd list --pretty${NC}"
    echo ""
    echo -e "${CYAN}════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}Phase 1: Orchestrator - Analyzing PRD with Beads${NC}"
    echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}"
    echo ""
    echo "  PRD: $(basename "$PRD_FILE")"
    echo "  Target tasks: $TARGET_TASKS"
    echo "  Beads directory: $BEADS_WORK_DIR"
    echo ""
    ...
    echo -e "${BOLD}${GREEN}════════════════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "View code:     ${CYAN}scraps clone $STORE/$REPO${NC}"
    echo -e "View commits:  ${CYAN}scraps log $STORE/$REPO${NC}"
    echo -e "View tasks:    ${CYAN}cd $BEADS_WORK_DIR && bd list --all --pretty${NC}"
    echo -e "Task graph:    ${CYAN}cd $BEADS_WORK_DIR && bd graph${NC}"
    echo ""
}
