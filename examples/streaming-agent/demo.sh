...
show_help() {
    ...
    echo "  OPENROUTER_MODEL     Model to use (default: google/gemini-2.0-flash-001)"
    echo "  SCRAPS_DEMO_STORE    Your store name (auto-detected if not set)"
    echo ""
    echo "Model options (set OPENROUTER_MODEL):"
    echo "  google/gemini-2.0-flash-001  - Fast & cheap (default)"
    echo "  deepseek/deepseek-chat       - Cheapest, good quality"
    echo "  anthropic/claude-3.5-haiku   - Reliable, slightly pricier"
    echo "  anthropic/claude-sonnet-4    - Best quality, most expensive"
    echo ""
    echo "The demo will:"
